...
class Application extends Koa {
  constructor(options={}) {
    ...
    const walk = p => helpers.walk(path.join(this.root, p))

    this.options = options
    this.root = options.root
    this.keys = options.keys
    this.controllers = walk('app/controller')
    this.models = walk('app/model')
    this.services = extend({}, services, walk('app/service'))
